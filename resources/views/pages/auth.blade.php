...
<main class="auth-page">
  ...
  <section class="auth-card {{ $isRegister ? 'is-register' : '' }}" data-auth-shell data-mode="{{ $activeMode }}"
    ...
    <div class="auth-form-panel auth-login-panel" data-auth-panel="login-form">
      <div class="auth-form-inner">
        ...
        <div class="auth-social-row" aria-label="Opsi login sosial">
          <a href="{{ route('auth.google') }}" class="auth-social-button" aria-label="Sign in dengan Google"
            data-cursor-hover>
        ...
        <form method="POST" action="{{ route('login.store') }}" class="auth-form" novalidate>
          ...
          <div class="auth-field">
            ...
            <input id="login-email" type="email" name="email" value="{{ old('email') }}" autocomplete="email"
              placeholder="you@afitech.id" required @error('email', 'login' ) aria-invalid="true"
              aria-describedby="login-email-error" @enderror>
          ...
          <div class="auth-field">
            ...
            <input id="login-password" type="password" name="password" autocomplete="current-password"
              placeholder="Masukkan password" required @error('password', 'login' ) aria-invalid="true"
              aria-describedby="login-password-error" @enderror>
    ...
    <div class="auth-form-panel auth-register-panel" data-auth-panel="register-form">
      <div class="auth-form-inner">
        ...
        <div class="auth-social-row" aria-label="Opsi register sosial">
          <a href="{{ route('auth.google') }}" class="auth-social-button" aria-label="Sign up dengan Google"
            data-cursor-hover>
            <svg viewBox="0 0 24 24" aria-hidden="true" focusable="false">
              <path fill="#4285F4"
                d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
        ...
        <form method="POST" action="{{ route('register.store') }}" class="auth-form" novalidate>
          ...
          <div class="auth-field">
            ...
            <input id="register-name" type="text" name="name" value="{{ old('name') }}" autocomplete="name"
              placeholder="Nama lengkap" required @error('name', 'register' ) aria-invalid="true"
              aria-describedby="register-name-error" @enderror>
          ...
          <div class="auth-field">
            ...
            <input id="register-email" type="email" name="email" value="{{ old('email') }}" autocomplete="email"
              placeholder="you@afitech.id" required @error('email', 'register' ) aria-invalid="true"
              aria-describedby="register-email-error" @enderror>
          ...
          <div class="auth-field">
            ...
            <input id="register-password" type="password" name="password" autocomplete="new-password"
              placeholder="Minimal 8 karakter" required @error('password', 'register' ) aria-invalid="true"
              aria-describedby="register-password-error" @enderror>
